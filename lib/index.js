...
const { rnd, hash, serialize, deserialize } = require('./helpers');
...
const main = (t) => {
	...
	if (o.msg == 'ping') {
	    ...
	    socket.send(serialize({id: t.id, msg: 'pong'}), rinfo.port, rinfo.address, (err) => {
		if (err)
	    ...
	    socket.send(serialize({id: t.id, msg: 'leader'}), rinfo.port, rinfo.address, (err) => {
		if (err)
		    console.log(t.id, err);
	    ...
	    socket.send(serialize({id: t.id, msg: 'leader ack'}), rinfo.port, rinfo.address, (err) => {
		if (err)
		    console.log(t.id, err);
	...
	if (!t.leader) { // if we don't know the leader
		...
		t.candidacyTimer = setTimeout(() => { // start my candidacy (after a random timeout)
			...
			socket.send(serialize({id: t.id, msg: 'candidate'}), peer[1], peer[0], (err) => {
			    if (err)
				console.log(t.id, err);
	    ...
	    // only ping if I am not the leader
	    if (!(t.leader.host == t.host && t.leader.port == t.port))
		socket.send(serialize({id: t.id, msg: 'ping'}), t.leader.port, t.leader.host, (err) => {
                    if (err)
			console.log(t.id, err);
